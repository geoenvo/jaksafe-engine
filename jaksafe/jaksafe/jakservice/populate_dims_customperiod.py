...
def get_data_from_dims_period(timestamp_t0, timestamp_t1):
    ####
    t0 = timestamp_to_formatted_date(timestamp_t0,dims_format_time)
    t1 = timestamp_to_formatted_date(timestamp_t1,dims_format_time)
    print "Starting crawling data from dims ..."
    time_conf = 'fromTime=%s&toTime=%s'%(t0, t1)
    dims_url = base_dims_url+'?'+ time_conf
    print "url dims => %s"%(dims_url)
    response = requests.get(dims_url)
    if response.status_code != 200:
...
def main():
    ...
    ## Default t0   
    t0_default = '2016080100' #### custom t0
    t1_default = '2016090100' #### custom t1
    timestamp_t0 = formatted_date_to_timestamp(t0_default,dims_format_time)
    timestamp_t1 = formatted_date_to_timestamp(t1_default,dims_format_time) ####
    ...
    dims_data = get_data_from_dims_period(timestamp_t0, timestamp_t1) ####
